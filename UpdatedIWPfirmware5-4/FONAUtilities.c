...
char ReceiveTextMsg[400]; //This is the string used to buffer up a message read from the FONA
...
int op_search_t = 180000; //amount of time it takes for +COPS to search for operators
...
int CheckNetworkConnection(void) //True when there is a network connection
{
    ...
    // case we are here for max 1.28sec when there IS a network connection and less if not   
        ...
        TMR1 = 0;
        while (netLightPin && LightIsFlashing) {
            if(TMR1 > 2000){
        ...
        TMR1 = 0;
        while ((!netLightPin)&&(LightIsFlashing)&& (!network_status)) {
            if(TMR1 > 18000){
                network_status = 1; 
            }
        }    
...
/*********************************************************************
 * Function: CheckNetworkConnection3G
 * Input: None
 * Output: 1 if network connected 0 if not
 * Overview: Measures the time from NETLight High to next High
 *           Spec:     800ms if Network = YES(high = 800ms low = 800ms)
 * 
 *           Spec:     ON if Network = NO 
 * 
 *           We look for a LOW for less than 1000ms and say network YES
 * 
 *            * Note: Timer speed dependent
 * TestDate: Not Tested
 ********************************************************************/
int CheckNetworkConnection3G(void) //True when there is a network connection
{
    // This function should only be called once we know the FONA is on.  
    // If the FONA is on, the NET light will blink so we should not get stuck here
    // waiting for 1's and 0's.  Just to be safe, leave if you wait too long for
    ...
    if(!netLightPin){
        ...
        // When LED is flashing, we have a connection
    }
    else{
        TMR1 = 0;  //start the timer
        while(TMR1 < 15625){
    ...
    return network_status;  // `True, when there is a network connection. 
                            // (NET LIGHT is ON 800ms, OFF 800ms.)  Constant
                            //  OFF when no connection)
                            //  Constant ON when trying to connect

}

/*********************************************************************
 * Function: sendDebugMessage(char message[50], float value){
 * Input: message is the text string to be printed
 *        value is a possible number to be printed - if value - -0.1 no number
 *                                                   is printed
 * Output: None
 * Overview: This function is called when the programmer wants to print a message
 *           to the Tx pin on the Control board which can be read by a 
 *           protocol analyzer such as the Digilent system.  
 *           If the global variable print_debug_messages == 0.  This routine is 
 *              skipped and nothing is printed
 *           If it is a value of 1 or higher, the function first turns off the
 *           Cell phone board if it is on.  Tests have shown that the Cell phone
 *           board gets messed up if it is on when non-AT messages are sent to 
 *           it. The message is sent to the Tx pin just as if it were a string
 *           to be sent to the Cell Phone board (which is now off).
 *           If the cell phone board was ON when the function was called, it it
 *           turned back on.
 *           
 *  * TestDate: Not Tested
 ********************************************************************/
...
int AreThereTextMessagesToRead(void) 
{
    ...
    if(success ==1){
        ...
        if(fourG){
        success = sendMessage("AT+CPMS=\"ME\"\r\n");
        }
        else{
        success = sendMessage("AT+CPMS=\"SM\"\r\n");    
        }
        if(success){
...
int ClearReceiveTextMessages(int MsgNum, int ClrMode) 
{
    ...
    if(success == 1){
        ...
        ReceiveTextMsgFlag = 0; //clear for the next message
        if (threeG){
            delayMs(3200); //This Delay fixed the problem of the FONA not
        }                  //deleting its text messages... DO NOT DELETE
        
        if (fourG){
        sendMessage("AT+CPMS=\"ME\"\r\n");
        }
        else{
        ...
        sendMessage("\x1A\r\n"); // this terminates an AT SMS command
...
int sendTextMessage(char message[160]) 
{
 ...
 //   The SIM should have been turned on prior to getting here;
    ...
    if(success){
        //char messageCStr[strlen(message)+1];
        //int x;
        //for (x = 0; x < sizeof(messageCStr); x++){
            //messageCStr[x] = message[x];
        //}
        //messageCStr[strlen(message)] = 0x1A;
...
int ReadSIMresponse(char expected_reply[10]){
    ...
    // ReceiveTextMsgFlag gets incremented each time the FONA board
    //     sends a 0x0A (Line Feed?).  Looks like we expect the LF
    //     on either end of the response from the FONA after it sends
    //     a message
 ...
 * Input: none
 * Output: none
 * Overview:  This is called if the Tech_At_Pump switch indicates that someone is
 *            there working on the pump and also each hour to see if there have 
 *            been any messages sent to the system which require action. 
 *            If a tech is at the pump, don't try as many times to see if
 *               there are messages because the routine will be called each time
 *               around the no_pumping loop rather than just once as part of
 *               the hourly activities
 * TestDate: 
 ********************************************************************/
void CheckIncommingTextMessages(void){
    ...
    if(FONAisON){
        ...
        while((!msg_remaining)&&(num_tries < 36)){
            ...
            if((!msg_remaining)&&(techNotAtPump)){// we did not have a msg yet and there is no tech at the pump
        ...
        if(msg_remaining){ //Only read messages if there are any to read
            int msgNum = 1; //2G boards start slot numbering at 1
            if(threeG || fourG) {
                msgNum = 0; //3G boards start slot numbering at 0
            }
